...
export const editScoreSheet = asyncHandler(async (req, res) => {
    ...
    const scoreSheet = await ScoreSheet.findByIdAndUpdate(
      req.params.id,
      req.body,
      { runValidators: true }
    );
      ...
      if (participant.Entry.toString() === req.body.EntryId.toString()) {
        if (
          !participant.submittedtables.some(
            (st) =>
              st.JudgeID.toString() === req.body.Judge.userId.toString() &&
              st.Table === req.body.Judge.table
          )
        ) {
          participant.submittedtables.push({
            JudgeID: req.body.Judge.userId,
            Table: req.body.Judge.table,
          });
        }
      }
    });
    ...
    await syncScoreTable(
      req.body.TimetablePartId,
      req.body.EntryId,
